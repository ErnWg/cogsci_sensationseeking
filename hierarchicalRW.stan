...
transformed data {
  ...
  vector[nBandits] initShock; //initialise shcok probabilites
  
  
  for (b in 1:nBandits)
  {initV[b] = payoff[b];} //is there a better way?
  initShock = rep_vector(0.5, nBandits); //initialise to 0.5
  ...
  for (s in 1:nSubjects){
    ...
    for (t in 1:nTrials){
      ...
      if (choice_stim[s,t] != 0) {
        ...
        //Update RW model
        EP[choice_stim[s,t]] =  EP[choice_stim[s,t]] + alpha[s] * (shock[s,t] - EP[choice_stim[s,t]]); 
      }
    }
  }
}
